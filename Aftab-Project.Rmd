...
```{r message=FALSE, warning=FALSE}
# emotion_morty <- tokens %>% 
#   inner_join(get_sentiments("nrc")) %>% 
#   filter(name == "Morty") %>%
#   count(sentiment, sort=T)
# 
# emotion_morty$name <- NULL
# 
# 
# ggplot(emotion_morty, aes(x=reorder(sentiment, n), y=n)) +
#   geom_bar(stat="identity", aes(fill=sentiment), show.legend=F) +
#   geom_label(label=emotion_morty$n) +
#   labs(x="Sentiment", y="Frequency", title="How is the overall mood in Rick&Morty?") +
#   coord_flip() 
```

Top words for Positive, Fear, Negative for Morty
```{r}
